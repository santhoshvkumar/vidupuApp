...
// At the end of your upload.php file, add a function to get the certificate file by leave ID

header('Access-Control-Allow-Origin: *');

// This function can be called to get certificate information
function getCertificateFileByLeaveId($leaveId, $type = 'Medical') {
    include('config.inc');
    
    // Determine column name based on certificate type
    $columnName = ($type === 'Medical') ? 'MedicalCertificatePath' : 'FitnessCertificatePath';
    
    // Query database for the certificate path
    $query = "SELECT $columnName FROM tblApplyLeave WHERE applyLeaveID = '$leaveId'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if ($path && $path !== 'null' && $path !== '') {
            return [
                'status' => 'success',
                'path' => $path,
                'fullPath' => $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/' . $path
            ];
        }
    }
...
// Handle direct access to this script for retrieving certificates
if (isset($_GET['action']) && $_GET['action'] === 'getCertificate') {
    if (!isset($_GET['leaveId'])) {
        echo json_encode(['status' => 'error', 'message' => 'Missing leaveId parameter']);
        exit;
    }
    
    $type = isset($_GET['type']) ? $_GET['type'] : 'Medical';
    $result = getCertificateFileByLeaveId($_GET['leaveId'], $type);
